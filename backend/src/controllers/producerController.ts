...
export const updateProducer = async (req: Request, res: Response, next: NextFunction) => {
    const producerId = Number(req.params.id);
    ...
    try {
        const result = await pool.query(
            `UPDATE producers 
            SET name = $1, product = $2, location = ST_SetSRID(ST_MakePoint($3, $4), 4326)
            WHERE id = $5
            RETURNING id, name, product,
                ST_Y(location::geometry) as latitude,
                ST_X(location::geometry) as longitude,
                created_at`,
            [name, product, longitude, latitude, producerId]
        );
        ...
        logger.info(`Producer updated: id ${producerId}`);
        res.json(result.rows[0]);
    } catch (err) {
        logger.error("Failed to update producer:", err);
        next(err);
    }
};